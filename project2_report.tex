...
It can be hard to discern which lines best explain the correlation by just looking at plots, and in fact a few of the residual standard errors also seem quite equal, but looking at the model errors in quantile-quantile plot we see a high degree of non linearity in the, i.e. normally distrubed standard errors, see figure \ref{fig:qqplot}.
\begin{figure}[H]
	\includegraphics[width=\linewidth]{qqplot.png}
	\caption{"QQ-plot for DGT technique, top row is from the linear model and bottom row from the \textit{Michaelis Mente model}"}
	\label{fig:qqplot}
\end{figure}
